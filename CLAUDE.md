...
| 案内役 | **ループちゃん**。運営者が作っている学習アプリ（非公開・名称未定）の相棒キャラ。絵は運営者の制作物で、**アプリより先にここで公開する**と運営者が判断（2026-09-14）。`public/loop/` の顔 2 枚（120×120 PNG）と、トップの手を振る絵 1 枚（520×733 WebP、2026-09-15 追加）だけを使う。どれも EXIF など画像以外の情報を消してから置く |
...
吹き出しは 3 か所です。**話し手はどれもループちゃんですが、口調は文の持ち主で決めます。**
...
- ループちゃんは「教える人」より「一緒に考える相棒」の話し方。決め台詞（「あなたの『わからない』を、
  一緒にほどこう。」）は**いまは使っていません**（セリフ集が「初回紹介や節目に限る」と指定している）
