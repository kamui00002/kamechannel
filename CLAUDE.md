...
#### ⚠ K9 が「緑だが測っていない」状態だった（2026-09-06・実際に踏んだ）

最初の K9 は **ページ全体**の HTML に註の先頭 20 字が含まれるかを見ていました。
これは通ります。**ただし `<meta name="description">` にも註の先頭が入っている**ので、
**本文の段落をまるごと消しても K9 は緑のまま**でした。陽性対照を取って初めて分かりました。

いまは `<p class="note">` の中身だけを取り出し、**段落数**と**本文**の両方を見ています。
確認済み: 段落を全部消す → 落ちる／段落数はそのままで中身を差し替える → 落ちる。

**教訓は K9 に限りません。** 「その値が他の場所にも書かれていないか」を確かめずに
検査を書くと、検査は通るのに何も見ていない状態になります。
**検査を足したら、必ずそれを壊して落ちることを確かめること。**
